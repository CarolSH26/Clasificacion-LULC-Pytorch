# -*- coding: utf-8 -*-
"""CUADERNO2_parte2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1JwuPXqfW18NdT0w9wHOtiJ73ZBVBf4-A
"""
...
# Autenticarse en su cuenta de Google
ee.Authenticate()

# Inicialice Earth Engine con el ID del proyecto
# Asegúrese de que el ID del proyecto sea correcto y que la API de Earth Engine esté habilitada para el proyecto.
ee.Initialize(project='ee-jandresmorap0508')

ISO = 'COL' # "COL" es el código ISO para Colombia
ADM = 'ADM1' # Equivalente a distritos administrativos

# Consulta de límites geográficos
...
geoboundary.sample(25)

shapeISO = 'CO-TOL'
fig, ax = plt.subplots(1, figsize=(10,10))
geoboundary[geoboundary.shapeISO == shapeISO].plot('shapeName', legend=True, ax=ax);
...
def generate_image(
   ...
   # porque las bandas de Sentinel-2 son números enteros de 16 bits
...
# Obtener la geometría de la forma para Tolima
region  = geoboundary.loc[geoboundary.shapeISO == shapeISO]
centroid = region.iloc[0].geometry.centroid.coords[0]
region = eec.gdfToFc(region) #geodataframe a la colección de características
...
image = generate_image(
    ...
    max_date='2023-12-31', # del año 2020 y 2023
    cloud_pct=20, # Filtrar imágenes con una cobertura de nubes >= 10,0 %
)
...
folder = 'Tolima'
task = export_image(image, shapeISO, region, folder)

task.status()

# Ruta de archivo de imagen
cwd = './drive/MyDrive/Tolima/'
tif_file = os.path.join(cwd, '{}.tif'.format(shapeISO))

if not os.path.isfile(tif_file):
  #tif_file ya está definido con la ruta completa, no es necesario redefinirlo
  tif_file = '{}.tif'.format(shapeISO)
  !gdown "1Pafy9_BpMbbsTdJekuXdE_Q31-o_NZjk" -O "$tif_file" # Descargar a la ruta correcta

#Abrir archivo de imagen usando Rasterio
image = rio.open(tif_file)
boundary = geoboundary[geoboundary.shapeISO == shapeISO]

# Trazar la imagen y el límite correspondiente
...
def generate_tiles(image_file, output_file, area_str, size=64):
    ...
    #Abre la imagen rasterizada usando rasterio
    raster = rio.open(image_file)
    width, height = raster.shape

    # Crea un diccionario que contendrá nuestros mosaicos poligonales de 64 x 64 px
    # Más tarde, convertiremos este diccionario en un DataFrame de GeoPandas.
    geo_dict = { 'id' : [], 'geometry' : []}
    index = 0

    # Hacer una ventana deslizante a través de la imagen rasterizada
    with tqdm(total=width*height) as pbar:
      for w in range(0, width, size):
          for h in range(0, height, size):
              ...
              # Crea una geometría con forma a partir del cuadro delimitador
              bbox = box(*bbox)
...
output_file = os.path.join(cwd, '{}.geojson'.format(shapeISO))
tiles = generate_tiles(tif_file, output_file, shapeISO, size=64)
...
# Save predictions
filepath = os.path.join(cwd, "{}_preds.geojson".format(shapeISO))
...
if not os.path.isfile(filepath):
  filepath = "{}_preds.geojson".format(shapeISO)
